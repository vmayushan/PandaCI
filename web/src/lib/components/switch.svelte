<script lang="ts">
	...
	type SwitchProps = SvelteHTMLElements['label'] & {
		color?: Color;
		label: string;
		name?: string;
		flex?: boolean;
...
<label
	...
	class={[
		className,
		// Base layout
		!flex && 'grid grid-cols-[1fr_auto] items-center gap-x-8 gap-y-1 sm:grid-cols-[1fr_auto]',
		flex && 'flex items-center *:data-[slot=control]:ml-4'
	]}
	{...api.getRootProps()}
	{...props}
>
	...
	<span
		class="data-disabled:opacity-50 text-base/6 text-zinc-950 sm:text-sm/6 dark:text-white"
		{...api.getLabelProps()}
	>
		{label}
	</span>
	<span
		...
		class={[
			...
			// Checked
			'data-[state="checked"]:bg-(--switch-bg) data-[state="checked"]:ring-(--switch-bg-ring) dark:data-[state="checked"]:bg-(--switch-bg) dark:data-[state="checked"]:ring-(--switch-bg-ring)',
			// Focus
			'focus:outline-hidden data-focus:outline-2 data-focus:outline-offset-2 data-focus:outline-blue-500',
			// Hover
			'data-hover:ring-black/15 data-hover:data-[state="checked"]:ring-(--switch-bg-ring)',
			'dark:data-hover:ring-white/25 dark:data-hover:data-[state="checked"]:ring-(--switch-bg-ring)',
			// Disabled
			'data-disabled:bg-zinc-200 data-disabled:opacity-50 data-disabled:data-[state="checked"]:bg-zinc-200 data-disabled:data-[state="checked"]:ring-black/5',
			'dark:data-disabled:bg-white/15 dark:data-disabled:data-[state="checked"]:bg-white/15 dark:data-disabled:data-[state="checked"]:ring-white/15',
	...
	>
		<span
			...
			class={[
				...
				// Checked
				'group-data-[state="checked"]:bg-(--switch) group-data-[state="checked"]:shadow-(--switch-shadow) group-data-[state="checked"]:ring-(--switch-ring)',
				'group-data-[state="checked"]:translate-x-4 sm:group-data-[state="checked"]:translate-x-3',
				// Disabled
				'group-data-[state="checked"]:group-data-disabled:bg-white group-data-[state="checked"]:group-data-disabled:shadow-sm group-data-[state="checked"]:group-data-disabled:ring-black/5'
			]}
		>
		</span>
	</span>
