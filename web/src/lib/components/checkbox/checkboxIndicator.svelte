<script lang="ts">
	...
	let { color = 'dark/zinc', class: className, ...rest }: CheckboxProps = $props();
</script>

<span class={[[base, colors[color]], className]} {...rest}>
	<svg
		class="stroke-(--checkbox-check) size-4 opacity-0 group-data-[state='checked']:opacity-100 sm:h-3.5 sm:w-3.5"
		viewBox="0 0 14 14"
		fill="none"
	>
		<path
			class="group-data-indeterminate:opacity-0 opacity-100"
			d="M3 8L6 11L11 3.5"
			stroke-width={2}
		...
		/>
		<path
			class="group-data-indeterminate:opacity-100 opacity-0"
			d="M3 7H11"
			stroke-width={2}
			stroke-linecap="round"
			stroke-linejoin="round"
